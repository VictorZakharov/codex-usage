...
- Activity-aware forecasts learn the local hours when quota has historically decreased, exclude observed off hours from the pace, and draw optional flat forecast segments while keeping one uniform time axis. A gap bounded by equal quota readings is treated as assumed flat time (including overnight); otherwise, the forecast falls back to elapsed time until all 24 clock hours have evidence.
